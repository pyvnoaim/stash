 ...
 *
 * Reading is all it does. Nothing here changes anything at an exchange, and a key made read-only
 * is the standing advice — there was a `cancel` for a sweeper that would retire an armed setup's
 * unfilled order, and both it and the sweeper are gone: the call was written, tested, and never
 * reached by a single line, while three comments described a `sweep.ts` that was never committed.
 * A GET-only module is also a smaller thing to hold a key for. Bring it back with the sweeper if
 * that idea comes round again — it is a dozen lines and a POST.
 *
...
export type Order = {
   ...
   *  doesn't, and there a `sell` may really be closing a long. What that costs is a closing order
   *  that reads as an entry — which is why fillsOf in push.ts asks whether the position grew rather
...
/** Everything resting on the futures book, across symbols. Uncached, unlike `positions`: an order
 *  is read to decide whether it is still there, and a 30-second-old book cannot say — a fill would
 *  be announced half a minute late, or an order that filled would still read as waiting. */
